    ...
    override suspend fun getTasksBySection(projectId: Long, sectionId: Long): Result<List<Task>> {
        return runSuspendCatching {
            withContext(Dispatchers.IO) {
                ...
                if (!response.isSuccessful) error("Failed to fetch tasks (HTTP ${response.code()})")
    ...
    override suspend fun createTask(projectId: Long, sectionId: Long, name: TaskName): Result<Task> {
        return runSuspendCatching {
            withContext(Dispatchers.IO) {
                ...
                if (!response.isSuccessful) error("Failed to create task (HTTP ${response.code()})")
                val dto = response.body() ?: error("Task creation returned empty body")
                val task = dto.toDomainTask(sectionId)
    ...
    override suspend fun updateTask(
        ...
        return runSuspendCatching {
            withContext(Dispatchers.IO) {
                ...
                if (!response.isSuccessful) error("Failed to update task (HTTP ${response.code()})")
    ...
    override suspend fun deleteTask(projectId: Long, sectionId: Long, taskId: Long): Result<Unit> {
        return runSuspendCatching {
            withContext(Dispatchers.IO) {
                ...
                if (!response.isSuccessful) error("Failed to delete task (HTTP ${response.code()})")
